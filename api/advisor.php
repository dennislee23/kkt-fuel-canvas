<?php
/**
 * fuelretail.kittykat.tech — Fuel Advisor backend (self-contained, streaming).
 *
...
const FUEL_ADVISOR_DEFAULT_MODEL      = 'claude-haiku-4-5';
const FUEL_ADVISOR_MAX_TOKENS         = 1024;  // 5-8 bullet answers need headroom
...
const FUEL_ADVISOR_RATE_WINDOW        = 60;
const FUEL_ADVISOR_API_URL            = 'https://api.anthropic.com/v1/messages';
...
const FUEL_ADVISOR_DAILY_CAP_DEFAULT  = 300;
const FUEL_ADVISOR_CACHE_TTL_DEFAULT  = 21600; // 6h
const FUEL_ADVISOR_ALLOWED_HOSTS      = ['fuelretail.kittykat.tech'];
...
// the change.
const FUEL_ADVISOR_FALLBACK_PROMPT = <<<'PROMPT'
You are an AI-transformation advisor for independent fuel networks. Speak in
business language — margin, stockouts and surpluses, working capital, railcar
demurrage, depot and station operations, loyalty and B2B. Be concrete and
grounded in the fuel network economy; never generic marketing. Stay under
120 words and answer in plain text only (no special tokens or JSON).
PROMPT;

const FUEL_ADVISOR_ALLOWED_ROADMAP_STEPS = ['essential', 'advanced', 'frontier'];


// ─── Entry: handle CORS, OPTIONS, method check ───────────────────────────────
$origin = $_SERVER['HTTP_ORIGIN'] ?? '';
$originAllowed = ($origin === 'https://fuelretail.kittykat.tech')
...
if (mb_strlen($message) > FUEL_ADVISOR_MAX_MESSAGE_LENGTH) {
    header('Content-Type: application/json');
...
try {
    ...
    error_log('fuel-advisor stream error: ' . $e->getMessage());
...
function stream_claude(string $apiKey, string $system, string $user, callable $onText, array &$usageOut = []): void
{
    $payload = json_encode([
        'model'      => load_model(),
        'max_tokens' => FUEL_ADVISOR_MAX_TOKENS,
        'stream'     => true,
        'system'     => [
    ...
    $ch = curl_init(FUEL_ADVISOR_API_URL);
    curl_setopt_array($ch, [
        CURLOPT_POST           => true,
        CURLOPT_POSTFIELDS     => $payload,
        CURLOPT_RETURNTRANSFER => false, // streaming via WRITEFUNCTION
        CURLOPT_TIMEOUT        => FUEL_ADVISOR_TIMEOUT_SECONDS,
        CURLOPT_HTTPHEADER     => [
...
function load_model(): string
{
    ...
    return $override !== '' ? $override : FUEL_ADVISOR_DEFAULT_MODEL;
}
...
function origin_referer_gate(): void
{
    ...
    foreach ($candidates as $u) {
        ...
        if (in_array($host, FUEL_ADVISOR_ALLOWED_HOSTS, true)
...
{
    return sys_get_temp_dir() . '/kkt_advcache_fuel_' . $key . '.json';
}

function response_cache_get(string $key): ?string
{
    if (load_env_int('ADVISOR_CACHE_TTL', FUEL_ADVISOR_CACHE_TTL_DEFAULT) <= 0) return null;
...
{
    $ttl = load_env_int('ADVISOR_CACHE_TTL', FUEL_ADVISOR_CACHE_TTL_DEFAULT);
    if ($ttl <= 0) return;
    @file_put_contents(
...
{
    $now = time();
    foreach (glob(sys_get_temp_dir() . '/kkt_advcache_fuel_*.json') ?: [] as $f) {
...
{
    $cap = load_env_int('ADVISOR_DAILY_CAP', FUEL_ADVISOR_DAILY_CAP_DEFAULT);
    if ($cap <= 0) return true; // disabled

    $day  = gmdate('Y-m-d');
    $path = sys_get_temp_dir() . '/kkt_advbudget_fuel.json';
    $fp = @fopen($path, 'c+');
    if (!$fp) return true;
    try {
...
function usage_log(string $mode, string $model, array $usage): void
{
    ...
    @file_put_contents(
        sys_get_temp_dir() . '/kkt_advisor_usage_fuel_' . gmdate('Y-m-d') . '.log',
 ...
 * Falls back to FUEL_ADVISOR_FALLBACK_PROMPT if the directory is empty.
 */
function load_system_prompt(): string
{
    $dir = __DIR__ . '/../knowledge';
    if (!is_dir($dir)) {
        return FUEL_ADVISOR_FALLBACK_PROMPT;
    }
    $files = glob($dir . '/*.md') ?: [];
    sort($files);
    $parts = [];
...
function rate_limit_check(): void
{
    ...
    $now  = time();
    $key  = sha1($ip);
    $path = sys_get_temp_dir() . '/kkt_ratelimit_fuel-advisor.json';

    $fp = @fopen($path, 'c+');
    if (!$fp) {
        error_log('fuel-advisor: rate limiter cannot open file — failing open');
        return;
    }
    ...
    try {
        ...
        $cutoff = $now - FUEL_ADVISOR_RATE_WINDOW;
        ...
        $stamps = $data[$key] ?? [];
        if (count($stamps) >= FUEL_ADVISOR_RATE_LIMIT) {
            $oldest     = $stamps[0] ?? $now;
            $retryAfter = max(1, ($oldest + FUEL_ADVISOR_RATE_WINDOW) - $now);
            ...
            echo json_encode([
                'error'       => 'rate_limited',
                'message'     => "Too many requests. Limit: " . FUEL_ADVISOR_RATE_LIMIT
                                 . " per " . FUEL_ADVISOR_RATE_WINDOW . "s.",
